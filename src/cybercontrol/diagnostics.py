"""Shared helpers for training-diagnostic plots and glossaries.

The companion repositories use different algorithms, but the reader-facing
diagnostics should use the same vocabulary.  This module keeps that small
common layer in the foundation package.
"""
...
COMMON_DIAGNOSTIC_TERMS: tuple[DiagnosticTerm, ...] = (
    DiagnosticTerm("iteration", "One optimizer, FBSM, or residual-update step.", "Use for solver or neural-training progress on the x-axis."),
    DiagnosticTerm("episode", "One complete sampled-data rollout used by RL/MARL training.", "Use for DDQN, PPO, MAPPO, or CTDE learning curves."),
    ...
    DiagnosticTerm("data loss", "Mismatch between the neural prediction and observed data.", "Shows data fit; it can improve while dynamics get worse if residual terms are weak."),
    DiagnosticTerm("ODE residual loss", "Mismatch between the neural time derivative and the ODE right-hand side.", "A PINN/PIDL equation-consistency check at collocation points."),
    ...
    DiagnosticTerm("boundary loss", "Mismatch at required initial or terminal boundary conditions.", "Important for PMP-informed models with terminal costate conditions."),
    ...
    DiagnosticTerm("correction regularizer", "Penalty that keeps a learned PIDL correction term small or smooth.", "Prevents the correction network from replacing the known mechanism."),
    ...
    DiagnosticTerm("collocation point", "A time or state-time point where a residual is enforced without requiring observed data.", "Controls where PINN/PIDL physics is checked."),
