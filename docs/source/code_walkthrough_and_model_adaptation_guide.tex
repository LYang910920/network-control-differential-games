...
\code{network_control_examples.py} is a compact extension script. It keeps the code in one readable file, but adds a degree-level attacker-defender differential game, node-level adjacency-matrix control and game models, a continuous-impulse simulation template, and additional input formats for realistic network data.
...
Run all tutorial examples from the repository root:

\begin{lstlisting}[style=bash]
python run_all.py --skip-reference
\end{lstlisting}

For the direct script commands below, move into the tutorial-example folder:
...
simple_outputs/
  ...
  degree_control_trajectory.png
  degree_control_baseline_comparison.png
\end{lstlisting}
...
example_outputs/
  ...
  degree_distribution.png
  degree_control_trajectory.png
  degree_game_trajectory.png
  node_control_trajectory.png
  node_game_trajectory.png
  continuous_impulse_trajectory.png
  degree_control_baseline_comparison.png
  degree_game_baseline_comparison.png
\end{lstlisting}
...
\subsection{Continuous-Impulse and impulse simulation}

The continuous-impulse example is a simulation template rather than a full optimal impulse solver. It illustrates the structure:
...
\]
This corresponds to the continuous-impulse jump form
\[
  x(t^+)=x(t)+g(x(t),v(t)).
\]
...
Continuous-impulse/impulse & Models combining continuous dynamics with instantaneous interventions \\
\bottomrule
\end{tabularx}
\end{center}
...
The continuous-impulse example currently simulates impulses at fixed times. To make impulse intensity optimal, add a search over admissible intensities. A minimal template is:
...
The teaching code is intentionally smaller than full paper-level code. The following public repositories provide useful research-style extensions. They correspond to the author's co-authored cyber/network-control publications: two papers in IEEE TIFS and one paper in IEEE TCSS.
...
\midrule
{\small\path{OpinionMalware_TIFS_2025_Code}} & IEEE TIFS 2025. Coupled malware-opinion dynamics with optimal impulse control. The repository contains \code{network.py} for multiplex-network construction and normalization, and \code{opinionMalware.py} for forward dynamics, backward adjoint equations, impulse-intensity search, and payoff computation. \\
\addlinespace
{\small\path{PropagandaWar_TIFS_2024_Code}} & IEEE TIFS 2024. Degree-based continuous-impulsive differential game between two propaganda campaigns. The repository contains \code{demo_network.py} for graph loading and degree distribution, \code{propWar.py} for forward-backward Nash strategy computation, and \code{comparison.py} for random-strategy Nash checks. \\
\addlinespace
{\small\path{Propaganda_TCSS_2025_Code}} & IEEE TCSS 2025. Optimal impulse control for suppressing cyber propaganda with awareness. The repository contains \code{prop_network.py} for network loading, \code{prop_propaganda.py} for forward/backward/policy iteration, \code{prop_cntrlComparison.py} for random impulse comparisons, and \code{prop_noAwareness.py} for an ablation baseline. \\
\bottomrule
\end{tabularx}
\end{center}
...
\begin{enumerate}
  ...
  \item Only after the degree-level version works, move to node-level, impulse, or continuous-impulse extensions.
\end{enumerate}
...
\bibitem{dgguide}
Differential games tutorial note provided by the user. The note introduces deterministic nonzero-sum open-loop Nash differential games, Hamiltonian functions, PMP necessary conditions, forward-backward sweep, and impulse differential games.

\bibitem{hybridguide}
Optimal hybrid control tutorial note provided by the user. The note introduces optimal control, impulse and continuous--impulsive hybrid control, jump dynamics, impulse Hamiltonians, and the three-layer structure of impulse-time and impulse-intensity optimization.
