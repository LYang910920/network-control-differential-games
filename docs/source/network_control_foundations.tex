...
\hypersetup{
  ...
  pdftitle={Optimal Control, Differential Games, Continuous-Impulsive Control, and Games on Networks},
  pdfauthor={Lu-Xing Yang},
  pdfsubject={Self-contained guide},
  pdfkeywords={optimal control, differential games, continuous-impulsive control, impulse control, networks, Python}
}
...
\pagestyle{fancy}
\fancyhf{}
\lhead{\small\sffamily Optimal Control, Games, and Continuous-Impulsive Control}
...
\centering
\vspace*{1.5cm}
{\sffamily\bfseries\Huge\color{TutorialBlue} Optimal Control, Differential Games, Continuous-Impulsive Control, and Games on Networks\par}
\vspace{0.5cm}
...
\vspace{0.8cm}
{\large Created from my research experience in optimal control, differential games, impulse and continuous--impulsive hybrid control, and cyber/network-security applications\par}
\vspace{0.35cm}
...
\textbf{Purpose.} This note starts from controlled dynamical systems and develops optimal control, differential games, and impulse and continuous--impulsive hybrid control in one coherent framework. It is written for propagation, defense, and intervention problems on networks. The mathematical sections introduce the state equation, payoff, Hamiltonian, PMP, HJB/HJI viewpoint, Nash equilibrium conditions, impulse jumps, and continuous-plus-impulse strategies. The computational sections give a simple degree-$k$ example first and then a main tutorial script for degree-level games, node-level adjacency-matrix models, and continuous-impulse simulations. The degree-level workflow always computes $N_k$, $P(k)$, and $\langle k\rangle$ from the input network before solving models indexed by degree class $k$.
\end{tcolorbox}
\vfill
{\large \today\par}
\end{titlepage}
...
This handout follows a layered structure. First, it defines a controlled dynamical system. Second, it formulates an optimal control problem for one decision maker. Third, it extends the same logic to two or more strategic decision makers, producing a differential game. Fourth, it adds discrete state jumps and event decisions, producing impulse and continuous--impulsive hybrid control/game models. Finally, it connects the mathematics to degree-level and node-level propagation models on networks and to runnable Python implementation.
...
\begin{itemize}
  ...
  \item \textbf{Continuous--impulsive hybrid control/game}: how should continuous-time controls and discrete impulse decisions be combined?
...
\begin{table}[h!]
\centering
\caption{A layered view of the guide.}
...
Continuous--impulsive hybrid control/game & Continuous dynamics plus impulse resets & How should continuous flow controls and impulse decisions be combined? & Routine monitoring plus occasional large-scale response \\
\bottomrule
\end{tabularx}
\end{table}
...
Action type & Continuous-time; impulse; continuous--impulsive hybrid & Continuous-time control changes derivatives; impulse control causes jumps; continuous--impulsive hybrid control contains both. \\
\bottomrule
\end{tabularx}
\end{table}
...
% -----------------------------------------------------------------------------
\section{Flow, Impulse, and Continuous--Impulsive Models}
% -----------------------------------------------------------------------------

The key distinction is how an action affects the state. A continuous-time control changes the derivative $\dot{x}$ over an interval of time. An impulse action causes an instantaneous state jump at selected time points. A continuous--impulsive hybrid policy uses both.

\subsection{Continuous-Time Control}

A continuous-time control $u(t)$ is applied over time and changes the vector field $f$. Examples include continuous patching intensity, monitoring rate, misinformation refutation effort, or resource allocation to recovery.
\begin{equation}
...
A continuous-time control signal is a function over a time interval and modifies the rate of change. An impulse action is an event action and modifies the state itself. In network security, continuous defense may be monitoring or patching; an impulse may be forced password reset, emergency cleanup, public warning, or manual shutdown of compromised accounts.
\end{definitionbox}

\subsection{Continuous--Impulsive Hybrid Control}

A continuous--impulsive hybrid control combines continuous-time flow controls $u(t)$ and impulse decisions $(N,t_l,v_l)$. In compact form,
\begin{align}
  \dot{x}(t)&=f(x(t),u(t),t),\qquad t\notin\{t_1,\dots,t_N\},\label{eq:contimp-flow}\\
  x(t_l^+)&=x(t_l^-)+g(x(t_l^-),v_l,t_l),\qquad l=1,\dots,N.\label{eq:contimp-jump}
\end{align}
The continuous--impulsive optimization problem is hard because it mixes continuous function optimization and discrete event optimization:
\begin{itemize}
...
Continuous--impulsive hybrid & Both derivative changes and jumps & $u(t)$, $N$, $t_l$, $v_l$ & Nested optimization; heuristics; mixed-integer/control methods \\
\bottomrule
\end{tabularx}
\end{table}
...
PMP is usually the most convenient route to \emph{open-loop} controls and open-loop Nash equilibria. HJB is the dynamic-programming route to \emph{feedback} controls. Impulse and continuous--impulsive hybrid models add jump conditions and event-time conditions to the continuous PMP system.
...
\subsection{Continuous--Impulsive Hybrid PMP and Nested Optimization}

Continuous--impulsive hybrid control combines two types of first-order conditions:
\begin{enumerate}
...
\end{enumerate}
For a fixed $N$ and fixed impulse schedule, the continuous--impulsive PMP is close to a continuous PMP system with extra jump equations. When $N$, impulse times, and target nodes are also optimized, the problem becomes a nested continuous-discrete optimization problem:
\begin{equation}
  \max_N\ \max_{0<t_1<\cdots<t_N<T}\ \max_{u(\cdot),v_1,\dots,v_N}\ J(u,v_1,\dots,v_N,t_1,\dots,t_N,N).
\end{equation}
This explains why many network propagation studies use restricted impulse schedules, candidate-time search, greedy target selection, or learning-based parameterized action spaces. These approaches do not replace the PMP; they make the PMP-inspired problem computationally tractable.

\subsection{Impulse and Continuous--Impulsive Hybrid Differential Games}

In a continuous--impulsive hybrid differential game, each player has its own continuous Hamiltonian and, if that player can trigger jumps, its own impulse Hamiltonian. For a two-player nonzero-sum game, player $i$ has
\begin{align}
...
\begin{enumerate}
  ...
  \item Specify whether controls are open-loop, feedback, sampled-data, impulse, or continuous--impulsive hybrid.
  ...
  \item For impulse or continuous--impulsive hybrid models, include state jump, adjoint jump, impulse Hamiltonian, and event-time condition if times are optimized.
...
\subsection{Continuous--Impulsive Node-Level Control: Continuous Patching plus Emergency Cleanup}
...
\begin{itemize}
  ...
  \item For continuous--impulsive policies, separate the value of continuous effort from the value of impulses by running ablation experiments.
...
\path{code/network_control_examples.py} & Main tutorial script containing degree-$k$ optimal control, degree-$k$ attacker-defender game, node-level optimal control, node-level game, continuous-impulse simulation, graph loading, adjacency conversion, and plotting. \\
...
The main tutorial script extends the same workflow to games, node-level models, and continuous-impulse simulations.  The command-line interface makes it possible to run all examples or only one group.
...
python code/network_control_examples.py --examples continuous-impulse
\end{lstlisting}
...
\midrule
OpinionMalware (IEEE TIFS 2025) & Repository \path{OpinionMalware_TIFS_2025_Code}; \path{network.py}, \path{opinionMalware.py}; network construction, normalization, coupled forward malware/opinion dynamics, backward adjoints, impulse strategy search, payoff. & Node-level coupled dynamics with impulses. The continuous-impulse simulation in the tutorial code is a minimal bridge before reading the full malware-opinion model. \\
PropagandaWar (IEEE TIFS 2024) & Repository \path{PropagandaWar_TIFS_2024_Code}; \path{demo_network.py}, \path{propWar.py}, \path{comparison.py}; graph loading, degree distribution, forward/backward Nash sweep, continuous-plus-impulse strategy update, random-strategy comparison. & Closest to the degree-$k$ differential-game example. The same blocks appear: $P(k)$ preprocessing, state equations, adjoints, bounded strategy updates, and payoff comparison. \\
Propaganda (IEEE TCSS 2025) & Repository \path{Propaganda_TCSS_2025_Code}; \path{prop_network.py}, \path{prop_propaganda.py}, \path{prop_cntrlComparison.py}, \path{prop_noAwareness.py}; network loading, impulse policy search, profit/payoff, random and no-awareness baselines. & Impulse-control workflow and ablation studies. The tutorial function \path{simulate_continuous_impulse} gives the basic state-jump structure before the full paper-specific implementation. \\
\bottomrule
\end{tabularx}
\end{table}
...
The file \path{reference_repository_guide.md} gives a reading map from the reference repositories to the mathematical ingredients of PMP, impulse control, and continuous--impulsive hybrid control.
...
\caption{Visual index of the foundation examples, including degree-level control/game, node-level variants, convergence diagnostics, and continuous-impulse behavior.}
...
Continuous--impulsive hybrid control & Flow controls plus explicit impulse resets. \\
...
\begin{itemize}
  ...
  \item \path{code/network_control_examples.py}: main tutorial script for degree-level games, node-level models, and continuous-impulse simulation.
  \item \path{README.md}: installation, commands, and usage notes.
  \item \path{reference_repository_guide.md}: mapping between the reference GitHub repositories and the mathematical workflow in this guide.
...
This two-file code structure keeps the tutorial readable: the first script shows the degree-$k$ PMP logic, and the main tutorial script shows how the same logic extends to games, node-level adjacency models, and continuous-impulsive interventions.
