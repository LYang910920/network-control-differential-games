...
\hypersetup{
  ...
  pdfauthor={Self-contained tutorial document}
}
...
\author{Self-contained tutorial document}
\date{\today}
...
After unzipping the tutorial package, move into the package root. A virtual environment is recommended:

\begin{lstlisting}[style=bash]
unzip Network_Optimal_Control_Differential_Games_Tutorial_Package.zip
cd network_control_game_lecture_package
...
Run the simple degree-\(k\) example first:
...
To download the reference repositories from the package, run:
