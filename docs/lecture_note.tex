...
% Colors and hyperref
% -----------------------------------------------------------------------------
\definecolor{DeakinBlue}{HTML}{1F4E79}
...
\hypersetup{
  colorlinks=true,
  linkcolor=DeakinBlue,
  citecolor=DeakinBlue,
  urlcolor=DeakinBlue,
  pdftitle={Optimal Control, Differential Games, Hybrid Control, and Games on Networks},
  pdfauthor={Luxing Yang},
  pdfsubject={Self-contained lecture note},
...
% -----------------------------------------------------------------------------
\titleformat{\section}{\Large\bfseries\sffamily\color{DeakinBlue}}{\thesection}{0.7em}{}
\titleformat{\subsection}{\large\bfseries\sffamily\color{DeakinBlue}}{\thesubsection}{0.7em}{}
\titleformat{\subsubsection}{\normalsize\bfseries\sffamily\color{DeakinBlue}}{\thesubsubsection}{0.7em}{}
...
\newtcolorbox{definitionbox}[1]{colback=SoftBlue,colframe=DeakinBlue,title=\textbf{#1}}
...
\centering
\vspace*{1.5cm}
{\sffamily\bfseries\Huge\color{DeakinBlue} Optimal Control, Differential Games, Hybrid Control, and Games on Networks\par}
\vspace{0.5cm}
{\sffamily\Large A Self-Contained English Lecture Note with Degree-Level and Node-Level Examples\par}
\vspace{1.2cm}
{\large Prepared from the uploaded Chinese lecture notes on differential games and optimal hybrid control\par}
\vspace{0.35cm}
{\large Companion Python files: \texttt{simple\_degree\_k\_control.py} and \texttt{network\_control\_examples.py}\par}
\vfill
\begin{tcolorbox}[width=0.86\textwidth,colback=SoftBlue,colframe=DeakinBlue]
...
\begin{table}[h!]
\centering
\caption{A layered view of the lecture note.}
...
The companion code implements this workflow in the single-file functions \texttt{load\_network}, \texttt{graph\_to\_model\_matrix}, and \texttt{degree\_distribution\_from\_graph}. The convention used by the dynamics is
\begin{equation}
...
\end{equation}
where $\widehat{u}_j$ is obtained from $H_{u_j}=0$ before applying bounds. This is exactly the form used in the network examples and the companion Python code.
...
\section{Python Companion Code: Simple First, Then Research-Code-Style Examples}
% -----------------------------------------------------------------------------
...
\[
\text{minimal degree-}k\text{ example} \quad \Longrightarrow \quad \text{compact research-code-style companion script}.
\]
...
\subsection{Companion Files}

In this public repository, the commands in this section assume that you first move into \path{examples/lecture/}. From the repository root, you can also run all lecture examples with \path{python run_all.py --skip-reference}.

\begin{table}[h!]
\centering
\caption{Code and guide files in the companion package.}
...
\path{code/network_control_examples.py} & Compact companion script containing degree-$k$ optimal control, degree-$k$ attacker-defender game, node-level optimal control, node-level game, hybrid/impulse simulation, graph loading, adjacency conversion, and plotting. \\
...
\subsection{Run the Companion Examples}
...
\begin{lstlisting}[language=bash,caption={Run the companion examples.},numbers=none]
...
\begin{table}[h!]
\centering
\caption{Mapping from the reference repositories to the companion examples.}
...
\midrule
\texttt{OpinionMalware\_TIFS\_2025\_Code} & \path{network.py}, \path{opinionMalware.py}; network construction, normalization, coupled forward malware/opinion dynamics, backward adjoints, impulse strategy search, payoff. & Node-level coupled dynamics with impulses. The hybrid simulation in the companion code is a minimal bridge before reading the full malware-opinion model. \\
\texttt{PropagandaWar\_TIFS\_2024\_Code} & \path{demo_network.py}, \path{propWar.py}, \path{comparison.py}; graph loading, degree distribution, forward/backward Nash sweep, hybrid strategy update, random-strategy comparison. & Closest to the degree-$k$ differential-game example. The same blocks appear: $P(k)$ preprocessing, state equations, adjoints, bounded strategy updates, and payoff comparison. \\
\texttt{Propaganda\_TCSS\_2025\_Code} & \path{prop_network.py}, \path{prop_propaganda.py}, \path{prop_cntrlComparison.py}, \path{prop_noAwareness.py}; network loading, impulse policy search, profit/payoff, random and no-awareness baselines. & Impulse-control workflow and ablation studies. The companion function \path{simulate_hybrid_impulse} gives the basic state-jump structure before the full paper-specific implementation. \\
\bottomrule
\end{tabularx}
\end{table}
...
\begin{figure}[p]
\centering
\includegraphics[width=0.82\textwidth]{../examples/lecture/results/simple_demo/degree_distribution.png}
\caption{Empirical degree distribution $P(k)$ computed before solving the degree-$k$ control problem.}
\label{fig:simple-degree-distribution}
\vspace{1em}
\includegraphics[width=0.82\textwidth]{../examples/lecture/results/simple_demo/degree_k_control.png}
\caption{Degree-$k$ optimal-control result from the simple script.}
\label{fig:simple-degree-control}
\end{figure}

\begin{figure}[p]
\centering
\includegraphics[width=0.82\textwidth]{../examples/lecture/results/examples_demo/degree_game.png}
\caption{Degree-$k$ attacker-defender differential game.}
\label{fig:degree-game-output}
\vspace{1em}
\includegraphics[width=0.82\textwidth]{../examples/lecture/results/examples_demo/hybrid_impulse.png}
\caption{Hybrid/impulse simulation with continuous ODE segments and state jumps.}
\label{fig:hybrid-impulse-output}
\end{figure}
\clearpage
% -----------------------------------------------------------------------------
...
\begin{itemize}
  ...
  \item \path{code/network_control_examples.py}: compact companion script for degree-level games, node-level models, and hybrid/impulse simulation.
  \item \path{README.md}: installation, commands, and usage notes.
  \item \path{reference_repository_guide.md}: mapping between the reference GitHub repositories and the mathematical workflow in this lecture note.
  \item \path{../reference/download_reference_repositories.sh}: optional script to download the reference repositories.
\end{itemize}

\begin{lstlisting}[language=bash,caption={Core commands for the companion code.},numbers=none]
pip install -r requirements.txt
