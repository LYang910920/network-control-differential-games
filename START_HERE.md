...
1. Open `docs/network_control_foundations.pdf` for the tutorial narrative and mathematical setup.
2. Run `python run_all.py --skip-reference --skip-scalability` to verify the foundation examples quickly.
3. Open `examples/foundations/results/README.md` to see what each generated figure means.
...
6. Run `python run_all.py --skip-foundations` to check the reference-code smoke runs.
...
| `examples/foundations/` | self-contained foundation examples and generated figures |
...
5. `examples/foundations/code/scalability_analysis.py`: synthetic scale-free timing experiment using `python-igraph`; degree-level is the default, and sparse node-level FBS is available with `python run_all.py --skip-reference --include-node-scalability`. The reports list the solver type so degree-level adaptive timings and sparse fixed-grid node-level timings are not confused.
