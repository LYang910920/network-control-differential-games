...
def save_results(out_dir: Path, k: np.ndarray, counts: np.ndarray, p: np.ndarray, kbar: float,
    ...
    plt.plot(t, X @ p, label="mean infection")
    plt.plot(t, U @ p, label="mean control")
    for j in sorted(set([0, len(k) // 2, len(k) - 1])):
        plt.plot(t, X[:, j], "--", label=f"x_k, k={int(k[j])}")
    plt.xlabel("time")
    plt.ylabel("state / control")
    plt.title(f"Degree-k optimal control; cost={cost:.2f}")
    plt.legend()
    plt.tight_layout()
